...
for epoch in range(num_epochs):
    ...
    model, train_losses = train_smn.train(model, loss_fn, optimizer, train_data_loader, batch_size, epoch, num_epochs, vocab, device, train_uids_rows)

    with torch.no_grad():
        val_R1, val_R2, val_R5, acc = evaluate_smn.evaluate(model, valid_data_loader, evaluate_batch_size, epoch, num_epochs, vocab, max_conv_utt_num, max_utterance_length, device, valid_uids_rows)
...
# Run on test data.
test_R1, test_R2, test_R5, test_acc = evaluate_smn.evaluate(model, test_data_loader, evaluate_batch_size, 0, num_epochs, vocab, max_conv_utt_num, max_utterance_length, device, test_uids_rows)
