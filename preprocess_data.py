...
def numberize(inp, vocab, max_utt_num , max_utt_length, dic, is_context):
    ...
    if is_context:
        ...
        words = inp.split()
        selected_context = words[:min(len(words), max_len)]
    ...
    pad_first_size = max_len - len(result)
    #pad_last_size = math.floor((max_len - len(result))/2)
    if len(result) < max_len:
        result = [0] * pad_first_size + result
        idf_features = [0] * pad_first_size + idf_features
        tf_features = [0] * pad_first_size + tf_features
    if len(result)!= max_len:
        print('errrrrorrr')
    ...
    return result   #dim seq*(numOffeatures+1)  here is 2(0:word indx, 1:idf, 2:tf)
